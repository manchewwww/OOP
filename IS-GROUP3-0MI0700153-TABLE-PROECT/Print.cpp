...
void Print::printRow(const std::vector<std::vector<Cell*>>& vector, const unsigned* lenghtsOfCell, const unsigned maxCol, const unsigned row, const unsigned rows) const
{
	for (size_t j = 0; j < maxCol; j++) {
		if (vector[row][j] != nullptr) {
			...
			printSpase(lenghtsOfCell[j] - vector[row][j]->getLenght());
		}
		else {
			printSpase(lenghtsOfCell[j]);
		}
		std::cout << " | ";
	}
}
...
void Print::printTable(const std::vector<std::vector<Cell*>>& table) const
{
	...
	unsigned* size = new unsigned[maxCol];
	for (size_t i = 0; i < maxCol; i++) {
		size[i] = getMaxLenghtOnCol(table, rows, i);
	}
	for (size_t i = 0; i < rows; i++) {
		printRow(table, size, maxCol, i, rows);
		std::cout << std::endl;
	}
	delete[] size;
}
